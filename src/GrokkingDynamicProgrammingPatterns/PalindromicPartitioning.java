...
public class PalindromicPartitioning {
    ...
    public static void main(String[] args) {
        System.out.println(PalindromicPartitioningBruteForce.findMPPCuts("abdbca"));
    }
}

class PalindromicPartitioningBruteForce {

    public static int findMPPCuts(String st) {
        return (findRecursive(st, 0, st.length() - 1));
    }

    private static int findRecursive(String st, int startIndex, int endIndex) {
        if (startIndex >= endIndex || isPalindrome(st, 0, endIndex)) {
            return 0;
        }
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            if (isPalindrome(st, startIndex, i)) {
                minimumCuts = Math.min(minimumCuts, 1 + findRecursive(st, i + 1, endIndex));
            }
        }
        return minimumCuts;
    }
...
class PalindromicPartitioningTopdownDynamicProgrammingwithMemoization {
    ...
    private static int findMPPCutsRecursive(Integer dp[][], Boolean dpIsPalindrome[][], String st, int startIndex, int endIndex) {
...
class PalindromicPartitioningBottomupDynamicProgramming{
